...
const GalleryPage = () => {
  ...
  const fetchUserPhotos = async () => {
    try {
      ...
      const photosData = await fetchPhotos(profileId, "desc");
      setPhotos(photosData === "No Photos To Display" ? [] : photosData);
    } catch (error) {
      setMessage("Error fetching photos.");
      console.error("Error fetching photos:", error);
  ...
  const handleFileUpload = async () => {
    ...
    try {
      ...
      // Attach album name if selected
      if (selectedAlbumId) {
        const selectedAlbum = albums.find(album => album.albumId === selectedAlbumId);
        if (selectedAlbum) {
          formData.append("albumName", selectedAlbum.albumName);
        }
      }
  ...
  const handleAddToProfilePicture = async (e) => {
    ...
    const updatedUserData = { profileUrl: selectedPhoto };

    setMessage("");
    setLoading(true);
    try {
      const response = await updateUserProfile(profileId, updatedUserData, dispatch);
      alert(response);
      navigate(`/profile`);
    } catch (error) {
  ...
  return (
    <div className="flex">
      <GalleryNavbar />
      <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-r from-blue-400 via-purple-500 to-pink-600 text-white p-6 ml-64 w-full">
        <h1 className="text-3xl font-semibold text-center mb-8">Your Photo Gallery</h1>
        <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-lg text-gray-800">
          <h2 className="text-xl font-semibold text-center mb-4">
            {isProfileEditing ? "Upload New Photo and Add to Profile Picture" : "Upload New Photo"}
          </h2>
          <div className="flex justify-center items-center gap-4">
            <input type="file" accept="image/*" onChange={handleFileChange} className="p-2 border border-gray-300 rounded-lg" />
            <button onClick={handleFileUpload} className="bg-blue-600 text-white p-2 rounded-lg hover:bg-blue-700" disabled={loading}>
              {loading ? "Uploading..." : "Upload Photo"}
            </button>
          </div>
        </div>

        {message && <p className="mt-6 text-xl text-center text-yellow-200">{message}</p>}

        <div className="mt-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {photos.length === 0 ? (
            <p className="col-span-full text-center text-lg text-gray-200">No photos available.</p>
          ) : (
            photos.map(photo => (
              <div key={photo.publicId} className="relative rounded-lg shadow-xl hover:scale-105 cursor-pointer" onClick={() => openModal(photo.photoUrl, photo.publicId)}>
                <img src={photo.photoUrl} alt="Photo" className="w-full h-64 object-cover rounded-lg" />
              </div>
            ))
          )}
        </div>
